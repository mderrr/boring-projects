...
const Gettext = imports.gettext.domain("update-manager")
...
const BELL_LOADING_ICON = "bell-plus"
const UPDATES_ICON = "bell-dot"
const NO_UPDATES_ICON =  "bell"
...
const INSTALL_PACMAN_UPDATES_COMMAND = "/usr/bin/alacritty --title 'Updating Pacman Packages' --command sudo pacman --noconfirm -Syu"
...
const UpdateManager = new Lang.Class({
	...
	_aurUpdateProcess_stream: null,
	_pacmanUpdateProcess_stream: null,

	_updateProcess_pid: null,

	_aurUpdateProcess_sourceId: null,
	_pacmanUpdateProcess_sourceId: null,
	...
	_init: function() {
		...
		this.updateIcon = new St.Icon({
            gicon: this._getCustomIcon(BELL_LOADING_ICON),
	...
	_updatePanelLabel: function () {
		if (this._aur_updates_ready && this._pacman_updates_ready) {
			...
			if (total_number_of_updates == 0) {
				...
				this.updateIcon.set_gicon( this._getCustomIcon(NO_UPDATES_ICON) )
				this.updateAllMenuItem.actor.reactive = false

			} else {
				this.panelButtonLabel.visible = true
				this.updateIcon.set_gicon( this._getCustomIcon(UPDATES_ICON) )
	...
	_readAurUpdates: function() {
		...
		this._finishReading(this._aurUpdateProcess_stream, this._aurUpdateProcess_sourceId)
	...
	_readPacmanUpdates: function() {
		...
		this._finishReading(this._pacmanUpdateProcess_stream, this._pacmanUpdateProcess_sourceId)
	...
	_getAvailableAurUpdates: function() {
		try {
			...
			this._aurUpdateProcess_sourceId = GLib.child_watch_add(0, pid, Lang.bind(this, function() {this._readAurUpdates()}))
	...
	_getAvailablePacmanUpdates: function() {
		try {
			...
			// We will process the output at once when it's done
			this._pacmanUpdateProcess_sourceId = GLib.child_watch_add(0, pid, Lang.bind(this, function() {this._readPacmanUpdates()}))
			this._updateProcess_pid = pid
	...
	_finishReading: function(process_stream, process_source_id) {
		// Free resources
		process_stream.close(null)
		process_stream = null
		GLib.source_remove(process_source_id)
		process_source_id = null
	},
...
function enable() {
	...
	Main.panel.addToStatusArea(UPDATE_MANAGER_NAME, updateManager, 4)
