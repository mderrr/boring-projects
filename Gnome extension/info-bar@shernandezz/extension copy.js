...
const MAIN_LOOP_REFRESH_RATE = 1
...
function readInfoOutput() {
	var resultList = []
	var out, size

	do {
		[out, size] = infoCommand_process_stream.read_line_utf8(null)
		if (out) resultList.push(out)

	} while (out)

	resultList = resultList.toString().split(SEMICOLON)

	unaPanelMenu._updateUna(resultList[0])
	cpuPanelMenu._updateCpu(resultList[1])
	memPanelMenu._updateMem(resultList[2])
	netPanelMenu._updateNet(resultList[3])
	dtePanelMenu._updateDte(resultList[4])
	
	// Free resources
	infoCommand_process_stream.close(null)
	infoCommand_process_stream = null
	
	GLib.source_remove(infoCommand_process_source_id)
	infoCommand_process_source_id = null

	GLib.spawn_close_pid(infoCommand_process_pid)
	infoCommand_process_pid = null
}

function executeInfoCommand() {
	if(infoCommand_process_source_id != null) {
		return // A check is already running, don't run it again
	}

	var [parseok, argvp] = GLib.shell_parse_argv( INFO_COMMAND )

	if (!parseok) { throw PARSE_ERROR }

	var [res, pid, in_fd, out_fd, err_fd]  = GLib.spawn_async_with_pipes(null, argvp, null, GLib.SpawnFlags.DO_NOT_REAP_CHILD, null)

	infoCommand_process_stream = new Gio.DataInputStream({
		base_stream: new Gio.UnixInputStream({fd: out_fd})
	})

	infoCommand_process_source_id = GLib.child_watch_add(0, pid, readInfoOutput)
	infoCommand_process_pid = pid
